...
def eliminacion_gaussiana(A: np.ndarray | list[list[float | int]]) -> np.ndarray:
    ...
    for i in range(0, n - 1):  # loop por columna
        ...
        for j in range(i + 1, n):
            ...
            elementos_fila = (n + 1) - i  # número de elementos en la fila a operar
            
            A[j, i:] = A[j, i:] - m * A[i, i:]

            contadorMulti_Div += elementos_fila
            contadorSumas_Restas += elementos_fila
    ...
    for i in range(n - 2, -1, -1):
        ...
        for j in range(i + 1, n):
            ...
            contadorMulti_Div += 1
            if j > i + 1:
                contadorSumas_Restas += 1
        solucion[i] = (A[i, n] - suma) / A[i, i]
        contadorMulti_Div += 1
        contadorSumas_Restas += 1

    logging.info(f"Se han realizado {contadorIntercambios} intercambios de filas.")
    logging.info(f"Se han realizado {contadorSumas_Restas} sumas/restas.")
    logging.info(f"Se han realizado {contadorMulti_Div} multiplicaciones/divisiones.")
